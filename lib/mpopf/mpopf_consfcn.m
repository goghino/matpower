function [h, g, dh, dg] = opf_consfcn(x, time_period, om, Ybus, Yf, Yt, mpopt, il, varargin)
...
if nargout == 2     %% contraints only
    g = om.eval_nln_constraint(x, 1, time_period);       %% equalities (power flow)
    h = om.eval_nln_constraint(x, 0, time_period);       %% inequalities (branch flow limits)
else                %% constraints and derivatives
    [g, dg] = om.eval_nln_constraint(x, 1, time_period); %% equalities (power flow)
    [h, dh] = om.eval_nln_constraint(x, 0, time_period); %% inequalities (branch flow limits)
    dg = dg';
    dh = dh';
end