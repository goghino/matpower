function plot_storage_results(mpc_solution)
    ...
    subplot(2,1,1)
    stairs(Pgen_discharge', 'LineWidth',5);
    hold on;
    stairs(1:N, [Pgen_storage_max ]', '--', 'LineWidth',3);
    grid on
    ...
    set(gca,'FontSize',20);
    subplot(2,1,2)
    stairs(Pgen_charge', 'LineWidth',5);
    hold on;
    stairs(1:N, [Pgen_storage_min]', '--', 'LineWidth',3);
    grid on
    ...
    set(fig,'defaultAxesColorOrder',map);
    subplot(2,1,1)
    stairs( [Pgen_storage ]', 'LineWidth',5);
    hold on;
    stairs(1:N, [Pgen_storage_max]', '--', 'LineWidth',3);
    stairs(1:N, [Pgen_storage_min]', '--', 'LineWidth',3);
    grid on
    title('Individual storage network injection power : trajectory (solid) and maximum (dashed)')
    xlabel('t [hours]')
    ylabel('Storage power [MW]')
    set(gca,'FontSize',20);
    subplot(2,1,2)
    plot(0:N, E_storage', 'LineWidth',5);
    hold on;
    plot(0:N, E_storage_max','--', 'LineWidth',3);
    grid on
    ...
    grid on
    title('Cumulative storage network injection power : trajectory (solid) and maximum (dashed)')
    xlabel('t [hours]')
    ylabel('Storage power [MW]')
    set(gca,'FontSize',20);
    subplot(2,1,2)
    ...
    grid on
    title('Cumulative storage energy level: trajectory (solid) and maximum (dashed)')
    xlabel('t [hours]')
    ylabel('Storage energy [MWh]')
    set(gca,'FontSize',20);
